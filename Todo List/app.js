...
let All_clear_btn = document.querySelector(".btn-dark");
let clear_btn = document.querySelector("#remove-by");
...
function eventListeners() {
  ...
  All_clear_btn.addEventListener("click", Clear_All);
  clear_btn.addEventListener("click", Delete_Category);
}
...
function addTodo(nextTodo) {
  if (todoList.firstElementChild !== null && todoList.firstElementChild.className === "alert alert-danger") {
...
function deleteTodosFromUI$Str(e) {
  if (e.target.className === "bi bi-file-x") {
    ...
    let content = e.target.parentElement.parentElement.firstElementChild.textContent.trim();
...
function Delete_Category(e) {
  let category_key = localStorage.getItem("selector");
